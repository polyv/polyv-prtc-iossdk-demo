...
#import "PRTCResolutionChoosePicker.h"
...
@interface PRTCMeetingRoomVC ()<PRTCEngineDelegate,
UICollectionViewDataSource,
UICollectionViewDelegate,
PRTCFileCaptureControllerDelegate>{
    
    NSMutableArray<PRTCStream*> *_canSubstreamList;
    AVAudioRecorder *recorder;
    NSTimer *levelTimer;
    PRTCFileCaptureController *_fileCaptureController;
}
...
@property (nonatomic , strong) PRTCEngine *rtcEngine;
...
@property (nonatomic, strong) PRTCRecordView *recordView;
@end

static NSString *roomCellId = @"roomCellId";

@implementation PRTCMeetingRoomVC
...
- (void)setupPrtcEngine {
    ...
    // 设置日志级别
    if (isLog == 1) {
        [_rtcEngine.logger setLogLevel:PRTCLogLevel_DEBUG];

    }else{
        [_rtcEngine.logger setLogLevel:PRTCLogLevel_OFF];

    }
    //设置视频分辨率
    _rtcEngine.videoProfile = videoProfile;
    //设置本地镜像
    _rtcEngine.mirrorMode = PRTCVideoMirrorModeEnabled;

    //设置远端渲染模式
    [_rtcEngine setRemoteViewMode:(PRTCVideoViewModeScaleAspectFit)];
    [_rtcEngine setPreviewMode:(PRTCVideoViewModeScaleAspectFill)];
    ...
    // 设置视频分辨率
    _rtcEngine.videoProfile = PRTCEngine_VideoProfile_480P;
...
- (void)createFileCapture {
    _fileCaptureController = [[PRTCFileCaptureController alloc] init];
...
- (IBAction)didMirrorAction:(UIButton *)sender {
    ...
    if (_rtcEngine.mirrorMode == PRTCVideoMirrorModeEnabled) {
        _rtcEngine.mirrorMode = PRTCVideoMirrorModeDisabled;
    }else if(_rtcEngine.mirrorMode == PRTCVideoMirrorModeDisabled){
        _rtcEngine.mirrorMode = PRTCVideoMirrorModeEnabled;
    }
}


#pragma mark -- PRTCEngineDelegate
/**退出房间*/
- (void)PRTCEngineDidLeaveRoom:(PRTCEngine *)manager{
    NSLog(@"%@", NSStringFromSelector(_cmd));
}

/**与房间的连接断开*/
- (void)PRTCEngineDisconnectRoom:(PRTCEngine *)manager{
    NSLog(@"%@", NSStringFromSelector(_cmd));
}

/**收到远程流*/
- (void)PRTCEngine:(PRTCEngine *)manager receiveRemoteStream:(PRTCStream *)stream{
    if (stream) {
        PRTCStream *delStream;
        for (PRTCStream *temp  in _remoteStreamList) {
...
/**远程流断开(本地移除对应的流)*/
- (void)PRTCEngine:(PRTCEngine *)manager didRemoveStream:(PRTCStream *)stream{
    PRTCStream *delete = [PRTCStream new];
    for (PRTCStream *obj in _remoteStreamList) {
...
/**新成员加入*/
- (void)PRTCEngine:(PRTCEngine *)manager memberDidJoinRoom:(NSDictionary *)memberInfo{
...
/**成员退出*/
- (void)PRTCEngine:(PRTCEngine *)manager memberDidLeaveRoom:(NSDictionary *)memberInfo{
    NSString *message = [NSString stringWithFormat:@"用户:%@ 离开房间",memberInfo[@"user_id"]];
    [self.view makeToast:message duration:1.5 position:CSToastPositionCenter];
}

/**非自动订阅模式下 可订阅流加入*/
- (void)PRTCEngine:(PRTCEngine *)channel newStreamHasJoinRoom:(PRTCStream *)stream{
...
///**非自动订阅模式下 可订阅流退出*/
- (void)PRTCEngine:(PRTCEngine *)channel streamHasLeaveRoom:(PRTCStream *)stream{
    [self.view makeToast:@"可订阅的流离开" duration:1.5 position:CSToastPositionCenter];
    PRTCStream *newS = [PRTCStream new];
    for (PRTCStream *tempS in _canSubstreamList) {
...
/**流 状态回调*/
- (void)PRTCEngine:(PRTCEngine *)manager didReceiveStreamStatus:(NSArray<PRTCStreamStatsInfo *> *)status{
    NSLog(@"%@", NSStringFromSelector(_cmd));
}

- (void)PRTCEngine:(PRTCEngine *)manager didChangePublishState:(PRTCEnginePublishState)publishState mediaType:(PRTCStreamMediaType)mediaType{
    NSString *titile = @"";
    NSString *tips = @"";
    switch (publishState) {
        case PRTCEnginePublishStateUnPublish:
            ...
            tips = @"未发布";
            break;
        case PRTCEnginePublishStatePublishing: {
            titile = @"发布中...";
            tips = @"发布中...";
            break;
        }
            
        case PRTCEnginePublishStatePublishSucceed:{
            titile = @"取消发布";
            tips = @"发布成功";
            if (mediaType == PRTCStreamMediaTypeScreen) {
                // FIXME: screen
        ...
        case PRTCEnginePublishStateRepublishing: {
            titile = tips = @"正在重新发布...";
            break;
        }
        case PRTCEnginePublishStatePublishFailed: {
            titile = @"重新发布";
            tips = @"发布失败";
            break;
        }
        case PRTCEnginePublishStatePublishStoped: {
            titile = @"重新发布";
            tips = @"发布已停止";
            if (mediaType == PRTCStreamMediaTypeScreen) {
...
- (void)PRTCEngine:(PRTCEngine *)manager streamConnectionFailed:(NSString *)streamId{
    
}

- (void)PRTCEngine:(PRTCEngine *)manager error:(PRTCError *)error{
    NSLog(@"%@", NSStringFromSelector(_cmd));
    switch (error.errorType) {
        case PRTCErrorTypeTokenInvalid:
            [self.view makeToast:[NSString stringWithFormat:@"token无效"] duration:3.0 position:CSToastPositionCenter];
            break;
        case PRTCErrorTypeJoinRoomFail:
            [self.view makeToast:[NSString stringWithFormat:@"加入房间失败：%@",error.message] duration:3.0 position:CSToastPositionCenter];
            break;
        case PRTCErrorTypeCreateRoomFail:
            break;
        case PRTCErrorTypePublishStreamFail: {
...
/**收到自定义消息的回调*/
- (void)PRTCEngine:(PRTCEngine *)manager receiveCustomCommand:(NSString *)fromUserID content:(NSString *)content{
    NSLog(@"%@", NSStringFromSelector(_cmd));
}

/**远端视频关闭的回调*/
//- (void)PRTCEngine:(PRTCEngine *_Nonnull)channel remoteVideoMute:(BOOL)remoteVideoMute {
//
//}

/**媒体播放器播放结束的回调*/
- (void)PRTCEngine:(PRTCEngine *)channel mediaPlayerOnPlayEnd:(BOOL)isEnd{
    NSLog(@"%@", NSStringFromSelector(_cmd));
}

/**网络连接状态变化的回调*/
- (void)PRTCEngine:(PRTCEngine *)manager connectState:(PRTCConnectState)connectState{
    NSLog(@"%@", NSStringFromSelector(_cmd));
}

/**音量回调 200ms一次：设置isTrackVolume=YES时可以接收各路流的音量*/
- (void)PRTCEngine:(PRTCEngine *)manager didReceiveAudioStatus:(PRTCAudioStats *)audioStatus{
    NSLog(@"%@", NSStringFromSelector(_cmd));
}

- (void)PRTCEngine:(PRTCEngine *)manager networkQuality:(NSString *)userId txQuality:(PRTCNetworkQuality)txQuality rxQuality:(PRTCNetworkQuality)rxQuality{
    if ([userId isEqualToString:self.userId]) {
        if (txQuality == PRTCNetworkQualityDisconnect) {
            _network.image = [UIImage imageNamed:@"single0"];
        }
        else if(txQuality == PRTCNetworkQualityPoorest || txQuality == PRTCNetworkQualityPoorer){
            _network.image = [UIImage imageNamed:@"single1"];
        }
        else if(txQuality == PRTCNetworkQualityPoor){
            _network.image = [UIImage imageNamed:@"single2"];
        }
        else if(txQuality == PRTCNetworkQualityGood){
            _network.image = [UIImage imageNamed:@"single3"];
        }
        else if(txQuality == PRTCNetworkQualityExcellent){
...
@discussion mixId。
*/
- (void)PRTCEngine:(PRTCEngine *)manager deleteMixStream:(NSString *)mixId{
    NSLog(@"%@", NSStringFromSelector(_cmd));
}
...
*/
- (void)PRTCEngine:(PRTCEngine *)manager queryMix:(PRTCMixResponse *)response{
    NSLog(@"%@", NSStringFromSelector(_cmd));
}
...
- (void)setupTopMenu{//@"旁路推流",@"push"
    ...
    PLVMenuView *menuView = [[PLVMenuView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - 130 - 10, 0, 130, 44 * titles.count) WithTitles:titles WithImageNames:images WithMenuViewOffsetTop:k_StatusBarAndNavigationBarHeight WithTriangleOffsetLeft:80];
...
//                break;
    ...
    [menuView showMenuEnterAnimation:PLVEnterAnimationStyleRight];
...
//播放本地视频录制
- (void)play{
    PRTCNativeRecordMediaVC *mediaVC = [[PRTCNativeRecordMediaVC alloc] init];;
...
- (void)cloudRecord{
    if (!_recordView) {
        _recordView = [[[NSBundle mainBundle] loadNibNamed:@"PRTCRecordView" owner:self options:nil]objectAtIndex:0];
        _recordView.layer.masksToBounds = YES;
        _recordView.layer.cornerRadius = 22.f;
        _recordView.frame = CGRectMake(20, 120, 125, 44);
        __weak typeof(self) weakSelf = self;
        _recordView.didSelectBlock = ^(PRTCRecordViewStatus status) {
            switch (status) {
                case PRTCRecordViewStatus_Start:
                    NSLog(@"start cloud record.");
                    [weakSelf record];
                    break;
                case PRTCRecordViewStatus_Stop:
...
//                    [weakSelf.rtcEngine stopRelay:nil];
                    ...
                    break;
//                case PRTCRecordViewStatus_Close:
//                    NSLog(@"close");
...
- (void)record{
    PRTCMixConfig *mixConfig = [[PRTCMixConfig alloc] init];
    //配置视频录制相关参数
    mixConfig.streams = @[];
//    mixConfig.pushurl = @[@"rtmp://rtcpush.ugslb.com/rtclive/PRtc-h4r1txxy12111151yketwz111"];
    mixConfig.layout = 1;
    mixConfig.layouts = @[];
    ...
    mixConfig.bucket = @"prtc-test";
    mixConfig.region =@"cn-bj";
    mixConfig.watertype = 1;
    mixConfig.waterpos = 1;
    mixConfig.waterurl = @"http://prtc-living-test.cn-bj.ufileos.com/test.png";
...
//    http://prtc-test.cn-bj.ufileos.com/000099_PRtc-h4r1txxy_rtc1605079948.mp4
}
...
- (void)setupCollectionView {
    ...
    [_collectionView registerNib:[UINib nibWithNibName:@"PRTCRoomCell" bundle:nil] forCellWithReuseIdentifier:roomCellId];
}
...
- ( UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    PRTCRoomCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:roomCellId forIndexPath:indexPath];
    cell.stream = _remoteStreamList[indexPath.item];
    
    __weak typeof(self) weakSelf = self;
    cell.muteComplete = ^(PRTCStream * _Nonnull stream, int type, BOOL mute) {
        switch (type) {
            case 0:
...
- (void)capturer:(PRTCFileCaptureController *)fileCaptureController didCaptureVideoPixelBufferRef:(CVPixelBufferRef)pixelBufferRef timestamp:(CMTime)timestamp {
    if (self.rtcEngine.enableExtendVideoCapture) {
        [self.rtcEngine publishPixelBuffer:pixelBufferRef timestamp:timestamp rotation:(PRTCVideoRotation_0)];
    }
}
