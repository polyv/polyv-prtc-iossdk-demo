...
typedef NS_ENUM(NSInteger, PRTCFileVideoCapturerStatus) {
    PRTCFileVideoCapturerStatusNotInitialized,
    PRTCFileVideoCapturerStatusStarted,
    PRTCFileVideoCapturerStatusStopped
};
...
@implementation PRTCFileCaptureController{
    ...
    PRTCFileVideoCapturerStatus _status;
...
- (void)startCapturingFromFileNamed:(NSString *)nameOfFile
                            onError:(PRTCFileVideoCapturerErrorBlock)errorBlock {
    if (_status == PRTCFileVideoCapturerStatusStarted) {
        errorBlock(@"Capturer has been started.");
        return;
    } else {
        _status = PRTCFileVideoCapturerStatusStarted;
    }
...
- (void)setupReaderOnError:(PRTCFileVideoCapturerErrorBlock)errorBlock {
...
- (void)stopCapture {
    _status = PRTCFileVideoCapturerStatusStopped;
...
- (void)readNextBuffer {
    if (_status == PRTCFileVideoCapturerStatusStopped) {
