...
@protocol PLVMenuViewDelegate <NSObject>
- (void)didselectItemIndex:(NSInteger)index;
@end

typedef NS_ENUM(NSInteger,PLVEnterAnimationStyle) {
    PLVEnterAnimationStyleNone,
    PLVEnterAnimationStyleRight,
    PLVEnterAnimationStyleTop,
};

typedef void(^PLVDidSelectBlock)(NSInteger index);

@interface PLVMenuView : UIView
...
@property (nonatomic, weak) id <PLVMenuViewDelegate>delegate;


/**
 选中item block 回调
 */
@property (nonatomic, copy) PLVDidSelectBlock didSelectBlock;
