...
@interface PLVMenuView ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>{
    CGRect _frame;
    CGFloat _triangleOffsetLeft;
    CGFloat _menuViewOffsetTop;
    BOOL _isHasTriangle;
    PLVEnterAnimationStyle _animationStyle;
}
...
- (instancetype)initWithFrame:(CGRect)frame WithTitles:(NSArray *)titles WithImageNames:(NSArray *)imageNames WithMenuViewOffsetTop:(CGFloat)top WithTriangleOffsetLeft:(CGFloat)left
{
    ...
    if (self) {
        _frame = CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, titles.count * 44 + 10);
...
- (void)delaDataWithTitles:(NSArray *)titles WithImageNames:(NSArray *)imageNames
{
    ...
    for (NSString *title in titles) {
        ...
        PLVMenuItem *item = [PLVMenuItem menuItemWithDict:dictM];
        item.fontSzie = 14;
        item.titleColor = [UIColor whiteColor];
        item.separatorColor = [UIColor whiteColor];
...
- (void)setTitleColor:(UIColor *)titleColor
{
    ...
    for (PLVMenuItem *item in self.dataArray) {
...
- (void)setSubViews
{
    ...
    [self.tableView registerClass:[PLVMenuCell class] forCellReuseIdentifier:IDETIFIRE];
...
- (void)showMenuEnterAnimation:(PLVEnterAnimationStyle)animationStyle
{
   ...
   _animationStyle = animationStyle;
    ...
    if (animationStyle == PLVEnterAnimationStyleRight) {
    ...
    }else if (animationStyle == PLVEnterAnimationStyleTop){
...
- (void)hidMenuExitAnimation:(PLVEnterAnimationStyle)animationStyle
{
    if (_animationStyle == PLVEnterAnimationStyleRight) {
        self.contentView.alpha = 1;
        [UIView animateWithDuration:0.2 animations:^{
            self.contentView.alpha = 0;
    ...
    }else if (_animationStyle == PLVEnterAnimationStyleTop){
        self.contentView.transform = CGAffineTransformMakeScale(1, 1);
        [UIView animateWithDuration:0.2 animations:^{
...
{
    PLVMenuCell *cell =  [tableView dequeueReusableCellWithIdentifier:IDETIFIRE];
    
    if (!cell) {
        cell = [[PLVMenuCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:IDETIFIRE];
    }
    cell.backgroundColor = [UIColor clearColor];
    PLVMenuItem *item = self.dataArray[indexPath.row];
    cell.menuItem = item;
    return cell;
}
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        _tableView.backgroundColor = MLClolor(82, 187, 255, 0.6);
