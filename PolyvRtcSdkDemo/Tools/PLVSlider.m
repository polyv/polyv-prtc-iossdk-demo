...
@interface PLVCircleLayer: CALayer
...
@property (nonatomic,strong)PLVCircleLayer *thumbLayer;
...
- (void)doInit
{
    ...
    _currentIdx = 0;
    _sliderStyle = PLVSliderStyle_Nomal;
    ...
    //滑块
    self.thumbLayer = [[PLVCircleLayer alloc] init];;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (PLVSliderStyle_Nomal == self.sliderStyle || PLVSliderStyle_Cross == self.sliderStyle) {
        //绘制竖直刻度短线
        ...
        if (PLVSliderStyle_Nomal ==self.sliderStyle) {
            startY = MAX(0, spindleScaleStartPoint.y-self.scaleLineHeight);
        }else if (PLVSliderStyle_Cross == self.sliderStyle) {
    ...
    }else if (PLVSliderStyle_Point == self.sliderStyle) {
        //绘制圆点型刻度分隔
        NSInteger lineNum = self.scaleLineNumber+1;
        CGFloat oneW = (W-self.thumbDiameter)/self.scaleLineNumber;
        CGFloat x = self.thumbDiameter * 0.5;
