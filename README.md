...
- [PRTCSDK_iOS-1.8.0.1](https://repo.polyv.net/ios/download/prtc/PRTCSDK_iOS-1.8.0.1+210511.zip)


# 1 描述
PRTCSDK_iOS.framework 是Polyv推出的一款适用于iOS平台的实时音视频 SDK，支持iOS 9.0及以上系统，提供了音视频通话基础功能，提供灵活的接口，支持高度定制以及二次开发。<br/><br/>
...
## 4.4 泛文娱
### 4.4.1 一对一社交
客户可以利用Polyv实时音视频云实现 QQ、微信、陌陌等社交应用的一对一视频互动
### 4.4.2 狼人杀游戏
...
```
Add Files to "Your targets" -> [`pwd`/PRTCSDK_iOS/Framework/PRTCSDK_iOS.framework]
```
...
### 5.2.1 导入 SDK 头文件
    //objective-c
    <PRTCSDK_iOS/PRTCSDK_iOS.h> 
    
    //swift
    import PRTCSDK_iOS
### 5.2.2. 设置 userId 和 roomId，获取AppID；
### 5.2.3 初始化PRTCEngine 并设置代理以接收相关回调信息；
    //objective-c
    PRTCEngine *engine = [[PRTCEngine alloc] initWithUserId:userId  appId:appId roomId:roomId]];
    engine.delegate = self;
    
     //swift
     PRTCEngine *engine = PRTCEngine.init(userId:userId, appId: appId, roomId:roomId , appKey: appKey, token:token)
...
* 配置视频录制相关参数，开始视频录制
        ...
        //objective-c
        PRTCRecordConfig *recordConfig = [PRTCRecordConfig new];
        ...
        //swift
        let recordConfig = PRTCRecordConfig.init()
        recordConfig.mainviewid = userId;   //主窗口位置用户id
        recordConfig.mimetype = 3;          //录制类型  1 音频 2 视频 3 音频+视频
        recordConfig.mainviewmt = 1;        //主窗口的媒体类型 1 摄像头 2 桌面
        recordConfig.bucket = "urtc-test";  //存储地址的名称
